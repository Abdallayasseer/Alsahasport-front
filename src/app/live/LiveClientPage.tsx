    ...
    <main className="min-h-screen pt-24 pb-32 md:pb-8 px-4 container mx-auto relative overflow-hidden w-full max-w-[1600px]">
      ...
      <div className="flex flex-col lg:flex-row gap-6 h-[calc(100dvh-180px)] min-h-[500px]">
        ...
        <div className="flex-1 flex flex-col gap-4">
             ...
             {/* Player Wrapper */}
             <div className="flex-1 relative bg-white/5 rounded-3xl overflow-hidden border border-white/10 shadow-2xl z-10">
                <VideoPlayer 
             ...
             <div className="px-2 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                 ...
                 <div className="hidden md:flex items-center gap-3">
                     <button 
                        ...
                        className="px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white font-bold flex items-center gap-2"
                     >
        ...
        {/* Sidebar (Desktop) - Static Show/Hide */}
        <div 
            className={`hidden lg:block ${isSidebarCollapsed ? 'w-0' : 'w-[320px]'}`}
        ...
        {/* Mobile Sidebar Overlay - Static */}
        {!isSidebarCollapsed && (
            <div className="fixed inset-0 z-[60] lg:hidden">
                <div className="absolute inset-0 bg-black/40" onClick={() => setIsSidebarCollapsed(true)} />
                <div className="absolute left-0 top-0 bottom-0 w-[85%] max-w-[320px] h-full bg-dark-base border-r border-white/5 z-[70]">
                    <div className="h-full">
                        <Suspense fallback={<div className="w-full h-full bg-dark-surface/50 rounded-2xl md:animate-pulse" />}>
                            <LiveSidebar isCollapsed={false} toggleCollapse={() => setIsSidebarCollapsed(true)} />
                        </Suspense>
                    </div>
                </div>
