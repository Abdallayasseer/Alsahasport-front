...
export default function LiveSidebar({ isCollapsed, toggleCollapse }: { isCollapsed: boolean, toggleCollapse: () => void }) {
  ...
  return (
    <div 
        className={`relative h-full bg-dark-surface/60 border border-white/5 rounded-2xl flex flex-col overflow-hidden ${isCollapsed ? 'items-center' : ''}`}
    >
        {/* Header & Toggle */}
        <div className={`p-4 border-b border-white/5 flex items-center shrink-0 w-full ${isCollapsed ? 'justify-center py-6 flex-col gap-4' : 'justify-between'}`}>
            {!isCollapsed && (
                <div className="flex items-center gap-3">
                    <h3 className="text-base font-black text-white flex items-center gap-2">
                        <Radio size={18} className="text-alsaha-green md:animate-pulse" />
                        القنوات
                    </h3>
                </div>
            )}
            ...
            <button 
                ...
                className="lg:hidden w-11 h-11 rounded-xl border border-white/5 bg-white/5 text-white/50 flex items-center justify-center"
            >
        ...
        {!isCollapsed && (
            <div className="p-3 shrink-0 w-full">
                <div className="relative">
                    <Search size={16} className="absolute end-3 top-1/2 -translate-y-1/2 text-white/30" />
                    <input 
                        type="text" 
                        placeholder="ابحث عن قناة..." 
                        className="w-full bg-black/20 border border-white/5 rounded-xl py-2.5 pe-10 ps-4 text-sm text-white focus:outline-none focus:border-alsaha-green/30 focus:bg-black/40 placeholder:text-white/20"
                    />
                </div>
            </div>
        )}
            ...
            {CHANNELS.map((channel) => {
                ...
                return (
                    <Link key={channel.id} href={`/live?channel=${channel.id}`} className="block w-full">
                        <div
                            className={`
                                flex items-center rounded-xl relative w-full
                        ...
                        >
                            <div className="relative flex items-center justify-center flex-shrink-0">
                                ...
                                {isActive && !isCollapsed && (
                                    <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-alsaha-green rounded-full border-2 border-dark-surface md:animate-pulse" />
                                )}
                            </div>
                            ...
                            {isCollapsed && (
                                <div className="absolute right-full rtl:left-full rtl:right-auto top-1/2 -translate-y-1/2 mr-4 rtl:ml-4 rtl:mr-0 px-3 py-1.5 bg-dark-base border border-white/10 rounded-lg text-xs font-bold text-white opacity-0 whitespace-nowrap pointer-events-none z-50 shadow-xl flex items-center gap-2">
                                    <span>{channel.name}</span>
                                    {isActive && <span className="w-1.5 h-1.5 bg-alsaha-green rounded-full md:animate-pulse" />}
                                </div>
                            )}
                        </div>
                    </Link>
